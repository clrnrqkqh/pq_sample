 ...
 * 
 * TODO:
 *   task1 : [기능추가]방향키 이동시에 행 자동이동 기능
 *           1) 소트시에 반전상태 유지 또는이동기능, 스크롤 이동유무 처리
 * 
 *   task2 : [기능추가]팝업메뉴 또는 외부함수로 복사기능 ( ctl+c )기능 제외
 *           pq.exportCopy() 함수 추가
 * 
 *   task3 : [기능추가] 체크박스 관련 기능 추가
 *            1) 비활성화 기능 cb 객체에 disable 기능
 *               cb ={
 *                       disalbe:"-1"  // 데이터 값이 "-1" 이면 비활성화 시킨다.
 *                    }
 *            2) 비활성화 상태인 경우 이벤트로 항목제거
 *            3) 체크박스 헤더 비활성화 기능 및 헤더 체크상태 취득기능
 *            4) 헤더체크박스 disabled 기능 신규 함수 추가(onHeaderCheckEnable)
 *            5) 각셀의 체크상태값을 취득하는 신규함수
 *            6) 헤더체크박스 위치 조정기능 ( 우측, 또는 상단)
 *               -column 정보에 2개 키값 신규 정위 disabled_cls:"", enable:true,false , header_pos:"right"
 * 
 *   task4 : [기능추가] 하나의 컬럼에 CheckBoxd와 Editor 공존하는 경우
 *          1) column.editor ={ cls:"pq-editor-focus-custom"} 클래스 존재하면 복합요소 기능
 *          2) 체크박스가 존재하는 경우는 체크박스선택시 edit기능은 무시하는 기능(fn._editCell())
 * 
 *  task5 : [기능수정] 옵션정보 초기값 변경기능
 *         1) 컬럼정렬기능 초기화 기능
 *         2) formulasModel 객체 사용유무 제어(formulasModel.on)활성화 된경우만 사용가능
 *         3) columnTemplate 기본값 수정
 *         4) fillHandle 기능 제거 
 * 
 *  task6 : [기능수정] excelExport 기능 수정 및 변경
 *        1) 엑셀 내보기시에 useLabel 설정된 경우 내용을 취득해서 전달한다.
 *        2) 엑셀용 병합셀 구성하는 함수(pq.ExceltoLetter) 함수신규작성
 *        3) [기능오류] excelExport시에 틀고정값이 0 인경우 오류건 수정
 * 
 *  task7 : [기능추가] 컬럼 변경 기능( 전체컬럼 변경기능)
 *        1) 타입변경시 초기화 처리시에 이전 정보를 해제하는 기능
 *        2) 컬럼표시/비표시 기능, 엑셀연동 공유
 * 
 *  task8: [기능수정] 소트기능 수정
 *        1) 빈 데이터는 맨 하단으로 이동시키는 기능  
 * 
 *  task-100: [기능추가 v7.5.0 >=] virtualX, virtualY  기능으로  스크롤 이동에 행 높이변동을 고정하는 기능
    ...
    mixin.GrpTree = {
        ...
        onCMInit: function() {
            var self = this,
                ...
                o = that.options,
                firstCol, M = o[model];
            if (M.titleInFirstCol) {
                firstCol = o.colModel[0];
                M.titleIndx = firstCol.dataIndx = firstCol.dataIndx || Math.random() + ""
            }
...
// 4)헤더체크박스 disabled 기능 신규 함수 추가
//  option={enable : true, false }
       onHeaderCheckEnable : function(option){
            let self = this,column = this.colUI || {},
                that = self.that, $inp = this.$inp;
            if (self.hasCboxHead() && $inp.length){
                self.colCB.enable = column.enable = option.enable;
                
                that.refreshHeader( ); // 헤더페이팅 
                that.refreshColumn ({dataIndx: column.dataIndx}); // 해당컬럼 페인팅
            }
        },
//<-- [End]
...
//수정내용> {task3}: [기능추가] 헤더체크 선택시 이벤트를 발생
            let self = this, that = self.that,
               cb = (self.colUI.cb || self.colCB.cb), 
                cbAll = cb.all,
                check =evt.target.checked === null ? true : evt.target.checked, 
                rowList = cbAll ? that.options.dataModel.data : that.pdata,
            ui = {
                source :"header",
...
// --> [START] 수정자: Kim changeha 수정일자:2020:09:23
//수정내용> {task3}: [기능추가] 체크박스 관련 기능 추가
// 체크박스 왼쪽/오른쪽( 타이틀이 존재하는 경우만) 
//  6) column 정보에 2개 키값 신규 정위 disabled_cls, enable , hckboxpos:"right"
                let  colUI = self.colUI||{};
                if (!$inp.length) {
                     let pos = colUI.hchkboxpos || "";  // "right" : 우측에 표시, "top":상단에 표시

                    if ( pos =="right" ){  // 오른쪽
                        $td.find(".pq-title-span").append('<input type="checkbox" />');
                    }else if ( pos =="top" ){  // 상단
                        $td.find(".pq-title-span").text("");
                        let html = "<input type='checkbox' /><br><span>" + (colUI.pq_title || colUI.title)+ "</span>";
                        $td.find(".pq-title-span").append(html);
                    }else{  // 좌측
                ...
                if (!self.$inp || $inp[0] != self.$inp[0]) {
                    ...
                    // enable 값이 flase 이면 속성을 변경한다.
                    if (colUI.enable === false){
                        $inp.attr('disabled', "disabled");
                    }
                   // 헤더 disabled_cls 키 값이 존제하면 활성화 여부를 변경한다.
                   if ( colUI.disabled_cls ){
                        colUI.enable === false?$td.find(".pq-title-span").addClass(colUI.disabled_cls):$td.find(".pq-title-span").removeClass(colUI.disabled_cls); 
                    }
//<-- [End]
                    self.refreshHeadVal();
                    if (ISIE) {
...
// 5) 각셀의 체크상태값을 취득하는 신규함수
// ui ={rowIndx:rowIndx }
        getCheckStatus : function(ui){
            ui = ui || {};
            var that = this.that,
                column = this.colCB || {},
                check = (column.cb || {}).check,
                cbId  = column.dataIndx,
                data = column.cb.all ? that.getData() : that.options.dataModel.data,
                rowIndx = ui.rowIndx;
                if (data[rowIndx] && data[rowIndx][cbId] === check){
                    return true;
                }
                return false;
        },
        // ui 객체 신규추가 {check: true | false }
         refreshHeadVal: function(ui) {
         // 헤더체크값을 함수로 변경하는 경우 { check : true, false }지정한다. 
            ui && (this.inpVal = ui.check);
            // 헤더 체크값을 변경한다.
...
// --> [START] 수정자: Kim changeha 수정일자:2020:09:24
//수정내용> {task6}: excelExport 기능 수정 및 변경
// 2) 엑셀용 병합셀 구성하는 함수(pq.ExceltoLetter) 함수신규직성
    var ExceltoLetter = pq.ExceltoLetter =  function (mc) {
        let  mcs = [],
        mcLen = mc.length;
        for (let  i = 0; i < mcLen; i++) {
            let mc_ = mc[i];
            let obj = toLetter(mc_.c1) + (mc_.r1 + 1) + ":" + toLetter(mc_.c2) + (mc_.r2 + 1);
            mcs.push(obj)
        } 
        return mcs;                   
    };
...
//수정내용> {task5}:[기능수정] 옵션정보 초기값 변경기능
    fni.options = {
        ...
        editModel: {
            ...
            clicksToEdit: 1,  //[기능수정]기본값 변경 2->1 원클릭으로 변경
            filterKeys: true,
            keyUpDown: true,
        ...
        selectionModel: {
            column: false,    // 기본값 true->false변경
            type: "row",      // "cell" 기본값을 "row" 변경
            onTab: "nextFocus",
            row: true,
            mode: "single"     // "block" 기본값을 "single" 변경
        },
        // 3) columnTemplate 기본값 수정
        columnTemplate: {  halign: "center", align: "right", hvalign: "center", valign : "center" },
        ///
        showBottom: true,  // 기본값을 true->false 변경 
        showHeader: true,
        showTitle: true,   // 기본값을 true->false 변경
        showToolbar: true,  // 기본값을 true->false 변경
        showTop: true,      // 기본값을 true->false 변경
        sortable: true,
        sql: false,
        stripeRows: true,
        title: "&nbsp;",
...
//<-- [End]
    ...
    fn.isEditableCell = function(ui) {
       ...
       // --> [START] 수정자: Kim changeha 수정일자:2020:09:23
       //수정내용> {task3}[기능추가]:1) 비활성화 기능 cb 객체에 disable 기능
       if ( objP.column && objP.column.type == "checkbox" ){
           let cb = objP.column.cb || {}, dataIndx =  objP.column.dataIndx;
           if ( cb.disable != undefined && rd[dataIndx] && cb.disable == rd[dataIndx]){
               return false;
           }
       }
       //<-- [End]
    ...
    fn._onClickCell = function(evt) {
        ...
        //console.log("_onClickCell===>",evt, objP);
    ...
    fn.onRemoteSuccess = function(response, textStatus, jqXHR, raiseFilterEvent, objP) {
        ...
        // 엑셀내려받기
        if ( objP.callback && objP.Export ){
            objP.data =  retObj.data;
            objP.callback( objP );
            that.hideLoading();
            that.loading = false;
            return;
    ...
    fn._editCell = function(_objP) {
        ...
        //[기능추가] CheckBox만 존재하는 경우 tab키 입력시 포커스 처리 ( edit닌 입력 안됨)
        if ( column.editfocus === false ){
/*            let $td = that.getCell(objCall),
            $inp = $td.find("input");
            if( $inp.length ){ 
...
// --> [START] 수정자: Kim changeha 수정일자:2020:09:23
//수정내용> {task4}:[기능추가] 2) 체크박스가 존재하는 경우는 체크박스선택시 edit기능은 무시하는 기능
        if ( evt){
            let $target = $(evt.target);
            if ($target.length  > 0 && $target.is("input") && column.type == 'checkbox') {
                return;
            }
        }
...
// --> [START] 수정자: Kim changeha 수정일자:2020:09:23
//수정내용> {task4}:[기능추가] 1) 하나의 컬럼에 CheckBoxd와 Editor 공존하는 경우
        let chk_edit = (ceditor ?  ceditor.cls : "") =="pq-editor-focus-custom",
            _checked;
        if( chk_edit ){
            let $td = that.getCell(EM.indices),
                $inp = $td.find("input");
            if ( $inp.length){
                _checked = ($inp.prop("checked") === true) ? "checked":"" 
            }
        };
//<-- [End]
        ///////////////
...
//수정내용> {task4}:[기능추가] 1)하나의 컬럼에 CheckBoxd와 Editor 공존하는 경우 
            if( chk_edit ){
                inp = "<input " + _checked + " type=checkbox />";
                $cell.prepend(inp);
            }
//<-- [End]
        ...
        // 한셀에 체크박스와 Edit동시에 존재하는 경우$cell.find(".pq-editor-focus") 추가
    ...
    fn.attr = function(objP) {
        var rowIndx = objP.rowIndx,
            ...
            finalAttr, $tr, $td, ret, rowData = objP.rowData || this.getRowData(objP);
        if (!rowData) {
            return {
                attr: null
            }
...
// --> [START] 수정자: Kim changeha 수정일자:2020:09:18
//수정내용> {task1}: [기능추가]방향키 이동시에 행 자동이동 기능
            // 상하이동, 좌우이동 여부 상태 변수
            let keymoveStatus = "updown";  // "updown", "leftright" 신규변수 추가
            ...
            if (keyCode == KCLEFT || keyCode == KCRIGHT || keyCode == KC.UP || keyCode == KC.DOWN || SM.onTab && keyCode == KCTAB) {
                ...
                if (keyCode == KCLEFT && !rtl || keyCode == KCRIGHT && rtl || keyCode == KCTAB && evt.shiftKey) {
                    ...
                    keymoveStatus =  "leftright"; // 좌우이동 기능정의
                ...
                if (obj) {
                    ...
                    this.select({
                        keymoveStatus:keymoveStatus,  // 이동상태값 신규추가 
                        rowIndx: rowIndx,
                        colIndx: obj.colIndx,
                        evt: evt
                    })
...
// --> [START] 수정자: Kim changeha 수정일자:2020:09:18
//수정내용> {task1}: [기능추가]방향키 이동시에 행 자동이동 기능(ctl+c) 전체선택
                let  type = SM.type, mode = SM.mode;
                if ( SM.type == "row" ){ // 행 타입인경우 열 타입으로 변경
                    type = "cell"; mode = "block";  // cell, block 모드로 변경
                };
                if (type == "row" && mode != "single") {
                    that.iRows.toggleAll({
                        all: SM.all
                    })
                } else if (type == "cell" && mode != "single") {
                    that.iSelection.selectAll({
                        type: "cell",
                        all: SM.all
                    })
                }
...
//<-- [End]
        ...
        keyDownInEdit: function(evt) {
            ...
            if (evt.keyCode == keyCodes.TAB || evt.keyCode == EM.saveKey) {
                //[기능추가] downFocus 기능 추가 ( 7.3.0)버전기능
                ...
                //[기능수정] 키 위래이동시에 clicksToEdit= 1 자동으로 포커스 이동
                // 7.3.0 버전은  keyUpDown 기본값 undefined 
                if (evt.keyCode == keyCodes.DOWN || evt.keyCode == keyCodes.UP) {
                    obj = this.incrRowIndx(rowIndxPage, colIndx, evt.keyCode == keyCodes.DOWN);
                    //[기능추가] 키 이동시에 자동으로 Edit 활성화 하는 기능
                    //let  onSave = evt.keyCode == keyCodes.TAB ? EM.onTab : EM.onSave;
                    //  keyDownInEdit(함수)
                    obj.edit = EM.onSave == "nextEdit" ? true : false;
                    return this.saveAndMove(obj, evt)
                }
                /* 7.2.0 이하 버전 코드 
        ...
        select: function(_objP) {
            ...
            that.scrollCell({
                ...
                if (evt.shiftKey && evt.keyCode !== $.ui.keyCode.TAB && SM.type && SM.mode != "single" && (areas.length || type_row)) {
                    ...
                    if (type_row) {
// --> [START] 수정자: Kim changeha 수정일자:2020:09:18
//수정내용> {task1}: [기능추가]방향키 이동시에 행 자동이동 기능(이벤트 추가)
                        // keyDownSelect 콜백함수롤 정의 해서 추후 사용 
                        // pq.on("keyDownSelect", function(e,ui){})
                        if (that._trigger("keyDownSelect", evt, _objP) == false){
                            that.focus({ rowIndxPage: rowIndxPage, colIndx: colIndx });
                            return;    
                        };
                        if (_objP.keymoveStatus == "leftright") {
                            // 좌/우 이동시에 셀 액티브 처리
                            that.Range({
                                        r1: rowIndx,
                                        c1: colIndx,
                        ...
                        // 로직구현(방향키 이동시에 반전표시기능)
                        that.iRows.removeAll ();  // 이전 선택정보를 전부 해제한다.
                        that.setSelection({ rowIndx: rowIndx, focus: false }); // 변경 행 반전처리 한다.
                        // 행 변경시에 한 셀 선택 상태로 변경
                        areas.forEach(function(range, idx) {
                            if( range.type =="cell" ){
                                iSel.clearOther ( {r1: rowIndx,  c1: colIndx});
    ...
    cGenerateView.prototype = {
        ...
        renderCell: function(objP) {
            var self = this,
                ...
                cellattr, cellcls, tmp, cellprop = (rowData.pq_cellprop || {})[dataIndx] || {},
                rowprop, freezeCols = o.freezeCols,
                render, columnBorders = o.columnBorders;
// --> [Edit] 수정자:kim changa 일시:2020/07/29
// 수정내용>엑셀보내기 기능 수정
            if ( column.prop ){
                dprop = column.prop;
            }
// <-- [Edit] kim changa 2020/07/29            
            if (!rowData) {
                return
            }
            if (!Export) {
            ...
            if (render = column.render) {
                ...
                if (dataCell && typeof dataCell != "string") {
                    ...
                    if ( Export && dataCell && Array.isArray(dataCell) ){
...
// --> [START] 수정자: Kim changeha 수정일자:2020:09:23
//수정내용> {task5}: [기능수정] 1) 컬럼정렬기능 초기화 기능
                align = column.halign || column.align || "center",    // [기능추가] 기본값으로 중앙
                hvalign = column.hvalign || "center",// [기능추가] 기본값으로 중앙
//<-- [End]
...
// 수정내용> dataUF Undefined 되어서 [] 초기화 한다(7.4.0버전 수정완료)
                dataUF = DM.dataUF || [],
...
// --> [START] 수정자: Kim changeha 수정일자:2020:09:24
//수정내용> {task6}:excelExport 기능 수정 및 변경
// 엑셀 내보기시에 useLabel 설정된 경우 내용을 취득해서 전달한다.
                if ( ui.Export ){
                    renderLabel && (text = renderLabel.call(grid, ui)||"");
                    if (text == null)
...
// --> [START] 수정자: Kim changeha 수정일자:2020:09:23
//수정내용> {task3}: [기능추가] 2)비활성화 상태인 경우 이벤트로 항목제거
                rowList = nodes.filter(function(rd) {
                    // 비활성화 상태이면 구성을 하지 않는다.
                    if( self.isEditable(rd, colCB, rd.pq_ri, that.colIndxs[diCB], diCB) ) {
                      return true;
                    }
                }).map(function(rd){
                    var oldRow = {},
...
(function($) {
    ...
    fni._create = function() {
        ...
        that._super();
// --> [START] 수정자: Kim changeha 수정일자:2020:09:24
//수정내용> {task5}:옵션정보 초기값 변경기능
//2) formulasModel 객체 사용유무 제어(formulasModel.on)활성화 된경우만 사용가능
        // o.formulasModel.on
        if ( o.formulasModel.on )
...
//<-- [End]
    ...
    fn._initTypeColumns = function(option) {
// --> [START] 수정자: Kim changeha 수정일자:2020:09:24
//수정내용> {task7}:[기능추가] 컬럼 변경 기능( 전체컬럼 변경기능)
// 1) 타입변경시 초기화 처리시에 이전 정보를 해제하는 기능
        let  _type = (option && option.type) ? option.type : null;
        if ( _type =="checkbox" && this.iCheckBox ){
            let _iCheckBox = this.iCheckBox;
            for ( let di in _iCheckBox  ){
                _iCheckBox[di].destroy();
                delete _iCheckBox[di]; 
            }           
        };
        var CM = this.colModel,
            i = 0,
        ...
        add: function(columns, ci, CM, source) {
            ...
            if (that._trigger("beforeColAdd", null, ui) !== false) {
                ...
                columns.forEach(function(col) {
                    if (col.type == "checkbox") that.iCheckBox[col.dataIndx] = new _pq.cCheckBoxColumn(that, col)
                });
            ...
            if (that._trigger("beforeColMove", null, ui) !== false) {
                source == "undo" || source == "redo" || that.iHistory.push({
                    callback: function(redo) {
                        ...
                        else self.move(howmany, to, fromindx, toParent, fromParent, "undo")
                    }
                });
...
// --> [START] 수정자: Kim changeha 수정일자:2020:09:24
//수정내용> {task7}: [기능추가] 컬럼 변경 기능( 전체컬럼 변경기능)
// 2)전체 컬럼제거기능 allRemove 변수추가
            let allRemove = false;
            if( source && source =="all") allRemove = true;
            ...
            if (that._trigger("beforeColRemove", null, ui) !== false) {
                ...
                columns = allRemove ?CM : CM.splice(ci, n); 
                columns.forEach(function(col) {
                    if (col.type == "checkbox") {
                        var di = col.dataIndx;
                        iCheckBox[di].destroy();
                        delete iCheckBox[di]
                   }
                });
                // 전체 삭제인 경우  처리
                if ( allRemove ){
                    that.option("dataModel.data",[] );
                    that.option("colModel",[]);
                    that.refreshDataAndView();                    
                    return;
                };
...
// col 객체 존재유무 체크, refresh 유무 추가
                ...
                // ui.refresh = false 이면 refresh 안함
                if ( ui.refresh === false) {
                    if ( ui.diShow.length > 0 || ui.diHide.length > 0 ) this.init();  
                    return;
                }
                if ( ui.diShow.length > 0 || ui.diHide.length > 0 ){
                that.refresh({  colModel: true });
...
// --> [START] 수정자: Kim changeha 수정일자:2020:09:23
//수정내용> {task1}:[기능추가] 행 선택시 반전표시 기능 수정 ( 헤더, 숫자컬럼등은 옵션으로 사용유무 )
            var that = this.that,
                r1 = range.r1,
                c1 = range.c1,
                r2 = range.r2,
                ...
                // overlay 선택시에 헤더와 숫자컬럼 표시여부 옵션추가( 기본값은 데이터영역만 반전표시)
               _layer =  that.options.layer ? that.options.layer : {all:false},
               _cls = (range && range.type == "cell") ? _cls + " cell" : _cls;  // 하나 셀인경우 셀단위로 처리(블럭처리안함)             
                cls   = _cls,             
...
// <-- [Edit] kim changa 2019/10/24
            ...
            // 헤더영역 선택반전 표시유무
            if ( _layer.all && that.options.showHeader != false) {
...
// --> [START] 수정자: Kim changeha 수정일자:2020:09:21
//수정내용> {task2}: [기능추가]복수줄인경우 복사기능을 위한 기능 추가
                var that = this.that,
                ...
                offset = that.riOffset,
                iGV = that.iRenderB;
              //복수줄인경우 복사기능을 위한 기능 추가
              let exportOption = that.options.exportOption||{}; 
              subItem = exportOption.subitem ? exportOption.subitem: 0;  // 복수줄 데이터 사용유뮤
              ////////////////////////////
            ...
            for (ri = r1; ri <= r2; ri++) {
                ...
                for (ci = c1; ci <= c2; ci++) {
                    ...
                    if (copy) {
                        ...
                        if (cv2 === cv) {
                            ...
                            // 복수줄인 경우 각각 분리를 한다.
                            if ( subItem > 1 ){  // 복수줄이상인 경우
                                let _cv;
                                !Array.isArray(cv2) && (_cv = Array.apply(null, Array(subItem-1)).map(function(){return ""}));
                                if( _cv ) cv2 = [cv2].concat(_cv);
                                cv2.forEach ( function(text){
                                    if (stringType[ci] && /(\r|\n)/.test(text)) {
                        ...
                        let _arData  = Array.isArray(cv2) ? cv2 : [cv2];
                        _arData.forEach ( function(cv,_idx ){
                            if ( !cells[_idx]) cells[_idx]=[];
                            cells[_idx].push(cv);
                        })
                        //rowBuffer.push(cv2);
                    }
                ...
                // 각 셀마다 데이터를 구성한다.
                cells.forEach ( function(cell, _idx){
                   str = cell.join("	");
                   buffer.push(str || " ");
                })
//<-- [End]
...
// --> [START] 수정자: Kim changeha 수정일자:2020:09:23
//수정내용> {task2}:[기능추가] 그리드내 복사기능인 경우 navigator.clipboard 사용으로 외부로 ctl+v 붙여넣기 기능 적용
                    // IE11 버전은 지원안함(navigator.clipboard)
                    navigator.clipboard && navigator.clipboard.writeText(str);
//<-- [End]
                }
            }
            if (dest) {
        ...
        // (kim changha)o.mergeCells 미 정의된 상태인 경우 추가 안되는 현상
        merge: function(ui) {
            ui = ui || {};
            var that = this.that,
                o = that.options,
            ...
            if (area) {
                ...
                if (rc > 1 || cc > 1) {
                    ...
                    area.cc = cc;
                    //mc.push(area);
                    if ( !o.mergeCells) o.mergeCells =[];
                        o.mergeCells.push(area);
                    if (ui.refresh !== false) {
                        that.refreshView()
                    }
                }
        ...
        _normal: function(range, expand) {
            ...
            var that = this.that,
                ...
                c2 = range.c2,
                tmp, type;
            if (r2 == null && rc == null && c2 == null && cc == null) {
                rc = 1;
                cc = 1
            }
            r2 = rc ? r1 + rc - 1 : r2, c2 = cc ? c1 + cc - 1 : c2, r2 = r2 > rmax ? rmax : r2, c2 = c2 > cmax ? cmax : c2, rc = r2 - r1 + 1;
            cc = c2 - c1 + 1;
            if (cmax < 0 || rmax < 0) {
                return null
            }
        ...
        if (rowListNew.length) {
            ...
            if (ui.refresh !== false) {
                ...
                if (len > 1) {
                    ...
                    if (keys.length == 1) that.refreshCell({
                        rowIndx: ri,
                        dataIndx: keys[0]
                    });
...
// --> [START] 수정자: Kim changeha 수정일자:2020:09:23
//수정내용> {task2}: [기능수정]복사기능 시에 초기값을 변경
        copyModel: {
            on: true,
            render: true,  // false->true 기본값 변경( column render 호출)
            header: true,  
            zIndex: 1e4
        },
//<-- [End]
...
// pq.exportCopy(ui)신규 함수추가 
        exportCopy: function(ui){
            ui = ui ||{};
            // navigator.clipboard  미지원 브라우저인경우 (외부로 ctl+v 붙여넣기 기능 적용) 
            if ( navigator.clipboard === undefined ){
                ...
                let $clip = $('#pq-grid-excel'); 
                $clip.select();
                $clip.focus();
                /* Copy the text inside the text field */
                document.execCommand("copy");
          }else{
              // navigator.clipboard 사용한다.
              this.copy();
          }   
        },
//<-- [End]      
...
//수정내용> {task1}:[기능추가]방향키 이동시에 행 자동이동 기능
// 1) 소트시에 반전상태 유지 또는이동기능, 스크롤 이동유무 처리
        let  _iRows = that.iRows,
            _selModel = options.selectionModel,     
            _ui = [], sortM = _selModel.sortRow || {}; // sortRow 신규생성
        if( _selModel.type == "row" && sortM.selMove ){  // 반전이동옵션기능
            _area = _iRows.getSelection()||[];
            // 이전 반전 상태 정보를 복사 한다.
            _ui   = _area.map(function(rd){
                return {rowIndx:rd.rowIndx};
            });
            //이전반전정보를 해제한다.
            _area.length && _iRows.removeAll ({all:true});
        };
        /////////////////////////////////////////////////////
//<-- [End]
        iSort.resumeSort();
...
// 1) 소트시에 반전상태 유지 또는이동기능, 스크롤 이동유무 처리
            // 소트변경후 재 반전표시를 한다.
            _ui.length && _iRows.add({rows:_ui});

            // scrollRow 지정값으로 위치이동
            if( _selModel.type == "row" && sortM.scrollRow ){
                // 중앙스크롤이동은 추후 고려
                let scrollY = sortM.scrollRow == "top" ?  0 : 0;  
                that.scrollRow ({rowIndxPage: scrollY });
            }
//<-- [End]
    ...
    var sortObj = {
        ...
        sort_number: function(obj1, obj2, dataIndx, dir) {
            var val1 = obj1[dataIndx],
                val2 = obj2[dataIndx];
// --> [START] 수정자: Kim changeha 수정일자:2020:09:24
//수정내용> {task8}:[기능수정] 소트기능 수정
// 1) 데이터가 빈데이터 이면 맨 마지막으로 이동 
            if(val1 === "" || val1 === null || val1 === undefined) return 1;
            if(val2 === "" || val2 === null || val2 === undefined) return -1;
//<-- [End]
...
(function($) {
    ...
    function cMerge(that) {
        ...
        this.mc = null;
        var self = this;
        that.on("dataReady colMove groupShowHide", function(evt, ui) {
    ...
    cMerge.prototype = {
        ...
        init: function(ui) {
            var that = this.that,
                ...
                data = that.get_p_data(),
                arr2 = [],
                exportMC = [],_pq_ri=[], row = 0,
                arr = [];
            // 숨김 행 데이터 미처리    
            if ( ui.Export && mc_o.length ){
                data.forEach ( function(rd, index){
                    if ( !rd.pq_hidden ){
                        _pq_ri[index] = row;
                        row++;
                    }
            ...
            for (var i = 0, len = mc_o.length; i < len; i++) {
                ...
                if ( ui.Export ){
                    // 실제컬럼인덱스 위치를 취득한다.
                    let colIdx = that.iCols.getvci(v_c1),
                    rowIdx = _pq_ri.length ? _pq_ri[v_r1] : v_r1;  
                    exportMC.push({
                        r1: rowIdx,
                        c1: colIdx,
                        rc: rs2,
        ...
        init: function() {
            ...
            if (!self._init) {
                ...
                that.on("cellClick", self.onCellClick(self)).on("cellKeyDown", self.onCellKeyDown(self, GM)).on(true, "cellMouseDown", self.onCellMouseDown()).on("change", self.onChange(self, GM)).on("dataReady", self.onDataReady(self, that)).on("beforeFilterDone", function() {
                    self.saveState()
                }).on("columnDragDone", self.onColumnDrag(self)).on("colMove", self.onColumnOrder(self, GM)).on("customSort", self.onCustomSort.bind(self)).on("valChange", self.onCheckbox(self, GM)).on("refresh refreshRow", self.onRefresh(self, GM)).on("refreshHeader", self.onRefreshHeader.bind(self));
                if (GM.titleIndx) {
    ...
    pq_options.dropModel = {
        ...
        drop: function(evt, uiDrop) {
            var Drag = uiDrop.helper.data("Drag"),
                ...
                top = uiDrop.ratioY() <= .5,
                rowIndx = uiDrop.rowIndx !== undefined ? uiDrop.rowIndx : this.options.dataModel.data.length - 1;
                rowIndx = rowIndx + (top ? 0 : 1);
                fn = function(G) {
                    var parent;
...
//수정내용> {task5}:[기능수정] 옵션정보 초기값 변경기능
//4) fillHandle 기능 제거 
    _pq.pqGrid.defaults.fillHandle = ""; // "all"에서 ""변경
//<-- [End]
    ...
    cTree.prototype = $.extend({}, pq.mixin.ChkGrpTree, pq.mixin.GrpTree, {
        ...
        moveNodes: function(nodes, parentNew, indx) {
            var self = this,
                ...
                DM = that.options.dataModel,
                pdata = that.pdata,
                dataOld, dataNew, children = parentNew[childstr],
                childrenLen = children.length,
                parentNewId, cache = {},
            ...
            if (len) {
                ...
                for (; i < len; i++) {
                    node = nodes2[i];
                    parentOld = self.getParent(node);
                    dataOld = parentOld ? parentOld[childstr] : pdata;
    ...
    cImport.prototype = {
        ...
        importS: function(sheet, extraRows, extraCols, keepCM, headerRowIndx) {
            var mergeCells = sheet.mergeCells,
                ...
                attr2, rowstyle, rowprop, tmp, propObj, styleObj, CM = that.options.colModel,
                hriOffset = 0,
        ...
        copyStyle: function(cell, prop, style) {
            ...
            // --> [Edit] 수정자:kim changa 일시:2020/07/29
            // 수정내용>엑셀보내기 기능 수정
            (tmp = cell.bgColor) && (style.background = tmp);
            // version >=7.2.0 background->background-color 변경
...
(function($) {
    pq.excelImport = {
        ...
        getFileText: function(path) {
            return path ? this.files[path.replace(/^\//, "")].asText() : ""
        },
        ...
        getWorkSheet: function($sheet, $sheetData, si, sheetName) {
            var self = this,
                key, cell, f, format, cell2, comments = self.comments,
                data = [],
...
// --> [Edit] 수정자:kim changa 일시:2020/07/29
// 수정내용>엑셀보내기 기능 수정 >= 7.2.0 background->background-color 변경
                if (tmp = s("background")) cell.bgColor = tmp;
...
// <-- [Edit] kim changa 2020/07/29
        ...
        Export: function(obj) {
            ...
            if (format == "xlsx") {
                ...
                let iCols = (obj.diShow || obj.diHide) ? that.Columns(): null , bCols = false;
                if ( iCols ){
                     iCols.hide({
                         diShow: obj.diShow,
                         diHide: obj.diHide,
                ...
                // 엑셀 내보내기 page sheet 
                var w;
                if ( obj.pagesheet && dataLen > 0 ){
                    w={sheets:[]};
                    let rPP = obj.PM.rPP ?  obj.PM.rPP : 20,
                    totalRecords = dataLen,
                    totalPages = Math.ceil(totalRecords / rPP ),
                    strRpp = obj.sheetName ? obj.sheetName : "page{0}";
                    for ( let curPage = 1 ; curPage <= totalPages; curPage++){    
                        let begIndx = (curPage - 1) * rPP;
                        begIndx = begIndx >= 0 ? begIndx : 0;
                        endIndx = curPage * rPP;
                        if (endIndx > totalRecords) {
                            endIndx = totalRecords;
                       }
                       let _data = data.slice(begIndx, endIndx),
                           _dataLen = _data.length,
                           _sheetName = strRpp.replace("{0}", curPage+"");
                       let w1 = self.getWorkbook(CMR, CMRLen, hc, hcLen, _data, _dataLen, remotePage, offset, iMerge, render, iGV, header, _sheetName );
                       that._trigger("workbookPageReady", null, {workbook: w1, PM:{curPage: curPage, totalPages:totalPages }});
                       
                       w.sheets.push(w1.sheets[0]);
                    }
                }else{
                    w = self.getWorkbook(CMR, CMRLen, hc, hcLen, data, dataLen, remotePage, offset, iMerge, render, iGV, header, obj.sheetName);
                }
        ...
        // #merge 조합
        // 복수 그리드를 하나의 엑셀싵로 보내기시에 병합셀 조정
        getXlsMergeStartIndex: function(){
            var self = this,
            o = self.that.options;
            //mcarr = o.mergeCells;

            return o.mergeRowIndex ? o.mergeRowIndex:0;
        },
        // 데이터 영역병합셀 재 구성을 한다.
        getxlsBodyMergeCells : function(hcLen, curPage, dataLen, iMerge ) {
            let mcarr = iMerge.init ({Export:true}) || [], 
            arr=[],
            that = this.that,
            r1, c1, offset = that.riOffset,
            offset2 = offset+ dataLen;
            mcarr.forEach ( function(mc){
                r1 = mc.r1;
                c1 = mc.c1;
                if (!curPage || r1 >= offset && r1 < offset2) {
                    if (curPage) {
        ...
        getXlsMergeCells: function(mc, hcLen, iMerge, dataLen) {
            // #merge 조합
            //console.log("getXlsMergeCells==>", mc );
            hcLen = hcLen + this.getXlsMergeStartIndex();
            mc = mc.concat(this.getxlsBodyMergeCells(hcLen, this.curPage, dataLen, iMerge));
            //mc = mc.concat(iMerge.getMergeCells(hcLen, this.curPage, dataLen));
            // 엑셀용 병합 셀 정보를 구성
            let mcs = pq.ExceltoLetter (mc);
            return mcs
        },
        // (kim changha)숨긴컬럼은 구성에서 제외
        getXlsCols: function(CM, CMLen) {
            var cols = [],
                col, column, width, i = 0, indx = 0,
            ...
            for (; i < CMLen; i++) {
                ...
                if (!pq.isEmpty(col)) {
                    ...
                    if ( !column.hidden ){
                        cols.length !== indx && (col.indx = indx);
                        cols.push(col); indx++;
                    }
                }
            }
        ...
        getXlsHeader: function(hc, hcLen, mc) {
            ...
            var self = this,
            // #merge 조합
            mergeRowIndex = self.getXlsMergeStartIndex(), 
            iCols = self.that.iCols,
            CM = self.that.colModel,
            CMLen = CM.length, 
            rows = [];

            //[기능추가] 헤더 동적으로 구성하기 위한 콜백함수 
            var getExportHeaderCell  =  function( cell, colIndx, rowIndx){
                let exRender = cell._excelRender; 
                if ( exRender && typeof exRender == "function" ){
                    let obj = exRender.call ( this.that , {
                        colIndx: colIndx,
                        column: cell,
                        rowIndx : rowIndx,
                        dataIndx: cell.dataIndx,
            ...
            for (var i = 0; i < hcLen; i++) {
                ...
                for (var ci = 0, lenj = row.length; ci < lenj; ci++) {
                    ...
                    var colspan = cell.colSpan, //cell.o_colspan,
                        rowspan = cell.rowSpan,
                        title = self.getTitle(cell, ci);

                    //[기능추가] 헤더 출력이 동적으로 변경되는 기능 적용
                    let objData = getExportHeaderCell ( cell, ci, i);   
                    if ( objData ){
                        colspan = objData.colSpan ? objData.colSpan:colspan;
                        rowspan = objData.rowspan ? objData.rowspan:rowspan;
                        title   = objData.title ? objData.title:title;
                    };
                    
                    let startRow = i+mergeRowIndex;// #merge 조합
                    let colIdx = iCols.getvci(ci);
                    if ( !objData && i > 0 && cell == hc[i - 1][ci]) {
                        title = "";
                    } else if (!objData && ci > 0 && cell == hc[i][ci - 1]) {
                        //title = "";  // [기능수정] 병합컬럼이 맨 처음 숨긴처리시에 이후 타이틀이 표시 안되는 현상
                        title = CM[ci-1].hidden ?  title : "";
                        if ( colIdx !== undefined && CM[ci-1].hidden && (colspan > 1 || rowspan > 1) ){          
                            mc.push({
                                r1: startRow,
                                c1: colIdx,
                                r2: startRow + rowspan - 1,
                                c2: colIdx + colspan - 1
                            })
                        }
                    } else if (colspan > 1 || rowspan > 1) {
                        if ( colIdx !== undefined && !cell.hidden ){          
                            mc.push({
                                r1: startRow,
                                c1: colIdx,
                                r2: startRow + rowspan - 1,
                    ...
                    let colHidden;
                    if ( CMLen > ci)
                        colHidden = CM[ci].hidden;
                        
                    // --> [Edit] 수정자:kim changa 일시:2019/07/10
                    // 수정내용> 엑셀내보내기 기능 ( <BR>요소를 "\r\n" 으로 변경)
                    if ( !cell.hidden && !colHidden){
                        // 숨긴셀은 구성에서 제외
                        cells.push({
                            //value: title(원본),
                            value: title.replace(/<br\s*[\/]?>/gi, "\r\n"),
                            //value: title.replace(/<br\s*[\/]?>/gi, ""), // 복수줄을 하나로 표현
                            align:"center",
                            valign:"center",
                            wrap:true,
                            bgColor: "#eeeeee"
                ...
                rows.push({
                    cells: cells
                    //wrap : self.that.options.hwrap ? true: false  // wrap값을 자동으로 변경 
                })
            }
            return rows
        },
        getExportOption: function ( ){
            let  self = this,
                that = self.that,
                exportOption = that.options.exportOption||{};  // 엑셀내보내기 옵션정보
        ...
        getXlsBody: function(CM, CMLen, data, dataLen, remotePage, offset, iMerge, render, iGV, shiftR, rowInit) {
            var self = this,
                ...
                cells, _cell, rowData, ri, rip, di, colStyle, colProp, row, cell, cellattr, cellattrdi, tmp, cellprop, cellpropdi, cellstyle, cellstyledi, shiftRC = _pq.cFormulas.shiftRC(that),
                format;
            debugger;
             let r_indx = 0 , exportOption = self.getExportOption(),  // 엑셀내보내기 옵션정보
             subItem = exportOption.subitem ? exportOption.subitem: 0;  // 복수줄 데이터 사용유뮤
            for (i = 0; i < dataLen; i++) {
                rowData = data[i];
                ...
                let indx = 0,m;  // 숨김컬럼은 제외
                for (j = 0; j < CMLen; j++) {
                    column = CM[j];
                    // [기능사양] 숨긴컬럼은 데이터 구성을 제외한다.
                    if ( column.hidden ) continue;
                    colStyle = column.style;
                    colProp = column;
                    di = column.dataIndx;
                    ...
                    value = rowData[di] || ""; // [기능사양] 
                    cv = value;
                    f = that.getFormula(rowData, di);
                    mergeCell = false;
                    // 병합셀 스타일 적용
                    _cell = {};
                    if (m=iMerge.ismergedCell(ri, j)) {
                        if (!iMerge.isRootCell(ri, j, "o")) {
                            mergeCell = true;
                            if ( subItem > 1 ){ // 병합셀인경우 복수줄 데이터 작성 
                                cv=Array.apply(null, Array(subItem)).map(function(){return ""});
                                ar=[cv];
                            }
                        };
                        if ( m.o_rc ){
                            let ui = iMerge.getClsStyle(ri, j);
                            // 병합셀 스타일 적용
                            if ( ui.style ){
                              self.copyStyle(_cell, ui.style );
                            }
                            // 병합된 셀에 부모행 히든된 경우
                            /*if ( m.o_ri !== ri  ){
                                objR.rowIndx     = m.o_ri;
                                objR.rowIndxPage = m.o_ri;
                                objR.rowData = data[m.o_ri];
                                value = cv = data[m.o_ri][di];
                                mergeCell = false;
                            }*/
                         };
                    };
                    ...
                    if (!mergeCell && !f) {
                        ...
                        if ( subItem > 1 ){
                            let _cv;
                            !Array.isArray(arr[0]) && (_cv = Array.apply(null, Array(subItem-1)).map(function(){return ""}));
                            if( _cv ) arr[0] = [arr[0]].concat(_cv);
                        }     
                        cv = arr[0];
                        dstyle = arr[1];
                        dprop = arr[2];
                    ...
                    // 배열처리로 변경
                    let _arData  = Array.isArray(cv) ? cv : [cv],
                        _dataLen = _arData.length; 
                    _arData.forEach ( function(cv,_idx ){
                        cv = cv || "";  // undefuned 인경우 빈문자열 설정
                        cell= (_idx == 0 )? _cell: {};
                        if ( !cells[_idx]) cells[_idx]=[];

                        if ( _dataLen > 1){
                            di = column._dataIndx ? column._dataIndx[_idx] : column.dataIndx;
                            if ( arr[1] ) dstyle = arr[1][_idx] || null; 
                            if ( arr[2] ) dprop  = arr[2][_idx] || null;
                            if ( arr[3] ) dtitle = arr[3][_idx] || null;
                        }

                        // [기능사양] pq_format 또는 pq_exlformat 사용
                        //  엑셀사용인 경우 pq_exlformat 객체를 사용한다.
                        let exlFormat = (rowData.pq_exlformat && rowData.pq_exlformat[di]) ? true : false;  
                        if ( exlFormat )
                            cell.format = rowData.pq_exlformat[di];
                        else 
                            format = self.getCellFormat(rowData, di) || column.format; // 기존소스

                        // pq_format이 존재 하는 경우만 사용
                        if (!exlFormat && typeof format == "string") {
                            if (pq.isDateFormat(format)) {
                        ...
                        if (!pq.isEmpty(cell)) {
                            ...
                            if ( !column.hidden ){
                                //cells.length !== indx && (cell.indx = indx);
                                // 한행에 복수 데이터 인경우 subitem 데이터 지정
                                if ( _dataLen > 1 )  cell.subitem = _idx;
                                cells[_idx].push(cell);
                                indx++;
                                //cells.push(cell);indx++;
                            }
                        }
                    });
                    // 임시로 작성한 _dataIndx 객체를 삭제를 한다. 
                    //if( column._dataIndx ) delete column._dataIndx;
                }
                row = {cells:[]};
                  // --> [Edit] 수정자:kim changa 일시:2019/07/10
                  // 수정내용> 엑셀내보내기 기능 ( <BR>요소를 "\r\n" 으로 변경)
                  ...
                  //  row.wrap = true; 
                  // <-- [Edit] kim changa 2019/07/10
                cells.forEach(function(_cells, _idx ){
                    //_cells.length && row.cells.push(_cells);
                    if( _cells.length){
                        row.cells[_idx] = [];
                        row.cells[_idx] = _cells;
                    }
                });
                //cells.length && (row.cells = cells);
                //[기능사양] 엑셀내보기시에 포맷정보는 pq_exlformat 객체를 사용
                // 사용후 객체를 제거한다.
                if ( rowData.pq_exlformat ) delete rowData.pq_exlformat;
                ...
                if (!pq.isEmpty(row)) {
                    ...
                    if ( !rowData.pq_hidden ){
                        //rows.length !== r_indx && (row.indx = r_indx);
                        row.cells.forEach( function(_row){
                            //rows.length !== r_indx && (_row.indx = r_indx);
                            rows.push({cells:_row});
                            r_indx++;
                        })
                        //rows.push(row);
                    }
    ...
    var pqEx = pq.excel = {
        ...
        exportWb: function(obj) {
            ...
            sheets.forEach(function(sheet, i) {
                var $cols = self.getCols(sheet.columns),
                    comments, hasComments, hasImage, ii = i + 1,
                ...
                let _st = self.getSheet($frozen, $cols, $body, $merge, hasComments, hasImage);
                //console.log(_st);
                xl.file("worksheets/sheet" + ii + ".xml", _st );

                if (hasComments) {
                    commentArr.push(ii);
                    xl.file("comments" + ii + ".xml", self.getComment());
                    xl.file("drawings/vmlDrawing" + ii + ".vml", self.getVml());
                    xl.file("worksheets/_rels/sheet" + ii + ".xml.rels", self.getSheetRel(ii))
                }
            });
            zip.file("[Content_Types].xml", self.getContentTypes(no, commentArr));
            xl.file("workbook.xml", self.getWBook(names));
        ...
        getBody: function(rows, columns) {
            var self = this,
                ...
                cellsLen, hidden, column, style, format, formatFinal;
            (columns || []).forEach(function(col, i) {
                colObj[col.indx || i] = col
            });
            for (i = 0; i < rowsLen; i++) {
...
// --> [Edit] 수정자:kim changa 일시:2020/07/29
// 수정내용>엑셀보내기 기능 수정(숫자 데이터를 문자형으로 변경하는 기능)
...
// <-- [Edit] kim changa 2020/07/29
        ...
        getContentTypes: function(no, commentArr) {
            var sheets = [],
                i = 1,
                comment = [];
            for (; i <= no; i++) {
...
// --> [START] 수정자: Kim changeha 수정일자:2020:09:25
//수정내용> {task6}: [기능수정] excelExport 기능 수정 및 변경
// 3) [기능오류] excelExport시에 틀고정값이 0 인경우 오류건 수정
            if ( r == 0 && c== 0 )
               return "";
//<-- [End]
        ...
        getSheetRel: function(i) {
            return '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">' + '<Relationship Id="rId2" Target="../comments' + i + '.xml" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/comments" />' + '<Relationship Id="vml' + i + '" Target="../drawings/vmlDrawing' + i + '.vml" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/vmlDrawing" /></Relationships>'
        },
        getSheet: function($frozen, $cols, $body, $merge, hasComments, hasImage) {
            return ['<?xml version="1.0" encoding="UTF-8" standalone="yes"?><worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships">', $frozen, $cols, "<sheetData>", $body, "</sheetData>", $merge, hasComments ? '<legacyDrawing r:id="vml1" />' : "", hasImage ? '<drawing r:id="drw1" />' : "", "</worksheet>"].join("")
        },
...
// --> [START] 수정자: Kim changeha 수정일자:2020:09:24
//수정내용> {task5}:[기능수정] 옵션정보 초기값 변경기능
// formulasModel을 사용하지 않는경우 예외처라
        if( this.iFormulas === undefined ) return undefined;
//<-- [End]
        ...
        exec: function(_fn, r, c) {
            var self = this,
                ...
                fn = cFormulas.deString(_fn, function(fn) {
                    ...
                    fn = fn.replace(/(?:[^:A-Z]|^)(\$?[A-Z]+\$?[0-9]+)(?!:)/g, function(a, b) {
...
// --> [START] 수정자: Kim changeha 수정일자:2020:09:24
//수정내용> {task4}:[기능추가] 하나의 컬럼에 CheckBoxd와 Editor 공존하는 경우
// 체크박스와 Edit 동시에 존재하는 경우 Edit영역 커스텀 
            let _cls = ui.column.editor ?  ui.column.editor.cls : "";
            if (editorType == "text" && _cls == "pq-editor-focus-custom" )
                widthTD = widthTD - 25;  // 넓이 조정
//<-- [End]
...
// 수정내용> 18로 고정된 부분을 부모 높이값으로 사용 
 ...
 // <-- [Edit] kim changa 2019/10/29
        ...
        position: function($editor, $grid, $td) {
            var $e = $editor.closest(".pq-editor-outer");
            $e.css("border-width", "0").position({
                my: "left center",
                at: "left center",
                of: $td,
                collision: "fit",
            ...
            if (this.that.options.rtl) $e.css("right", $td.css("right"))
        }
    }
})(jQuery);
(function($) {
...
(function($) {
    ...
    cVirtual.prototype = {
        ...
        // --> [START] 수정자: Kim changeha 수정일자:2020:09:24
        //수정내용> {task100}:[기능추가 v7.5.0 ] virtualX, virtualY  기능으로  스크롤 이동에 행 높이변동을 고정하는 기능
...
(function($) {
    ...
    pq.cRenderBody.prototype = $.extend({}, new $.paramquery.cGenerateView, new pq.cRender, {
        ...
        setPanes: function() {
            ...
            //console.log("$cr before==>", $cr.height(), cr.clientHeight, htBody, JSON.parse(JSON.stringify(dims)) );                
            ...
            //console.log("$cr after==>", $cr.height(), cr.clientHeight );                
            ...
            //console.log("$cl before==>", $cl.height(),clientHeight );
            // 틀고정 존재하면서 가로 스크롤이 정상적으로 표시 안되는 현상건 수정사항
            if ( self.width !== "flex" && wdLeftPane > 0 && (dims.htCont - clientHeight) == 0 ){
                if( clientWidth < dims.wdTbl )  // 가로스트롤 영역 존재유무
                    dims.htContClient = clientHeight= dims.htCont- sbDim;
            }
            ...
            //console.log("$cl after==>",  $cl.height(),clientHeight ,JSON.parse(JSON.stringify(dims)));
