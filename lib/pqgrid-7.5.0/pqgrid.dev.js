    ...
    mixin.GrpTree = {
        ...
        onCMInit: function() {
            ...
            if (M.titleInFirstCol && CM) {
                //
                if(typeof M.titleInFirstCol == "string" ){
                    firstCol = CM.find(function(col) {
                        return col.dataIndx == M.titleInFirstCol;
                    });
                }else{
                    firstCol = CM.find(function(col) {
                        return !col.hidden
        ...
        renderCB: function(checkbox, rd, cbId) {
            ...
            let   o = that.options,
            GM = o.groupModel;       
            if(GM.bodycheckbox === false &&  !rd.pq_gtitle) return "";
            /////////////**
    ...
    fn._digestDelete = function(rowList, iUCData, track, data, dataUF, PM, remotePaging, offset) {
        ...
        for (; i < len; i++) {
            var rowListObj = rowList[i],
                rowData = rowListObj.rowData,
                uf = false,
                rowIndx = data.indexOf(rowData);
            if (rowIndx == -1) {
                rowIndx = dataUF.indexOf(rowData);
                if (rowIndx >= 0) {
                    uf = true
                }
            }
    ...
    cCheckBoxColumn.prototype = $.extend({
        cellRender: function(colCB, colUI) {
            ...
            return function(ui) {
                ...
                let GM = self.that.options.groupModel,
                cb_gTitle = GM.cbId == diCB ? true :false ;
                if ( cb_gTitle && !rd.pq_gtitle ){
                    return "";
                }
    ...
    _pq.cColModel.prototype = {
        ...
        remove: function(n, ci, CM, source) {
            ...
            if (that._trigger("beforeColRemove", null, ui) !== false) {
                ...
                (columns = CM.splice(ci, n)).forEach(function(col) {
        ...
        refreshColumns: function() {
            ...
            if (GM_on) {
                if (GM.titleIndx) {
                    ...
                    if (typeof GM.titleInFirstCol =="string" || ( fixCols && !GM.titleInFirstCol )) {
...
(function($) {
    ...
    cContext.prototype = {
        ...
        getItemHtml: function(item, i) {
            if (item == "separator") {
                return "<tr class='pq-cmenu-item'><td colspan=4 class='pq-bg-3' style='height:0px;'></td></td>"
            } else {
            ...
            if (items && items.length) {
                if (m.preInit.call(that, evt, ui) !== false) {
                    ...
                    $menu.position({
                        ...
                        at: "left+" + (evt.pageX - offset.left) + " top+" + (evt.pageY - offset.top),
                        of: $ele,
                        collision: "flipfit"
                    });
...
(function($) {
    ...
    var cTree = _pq.cTree = function(that) {
        ...
        self.di_prev;
        self.chkRows = [];
        Object.defineProperty(that.options.treeModel, "nodeClose", {
            get: function() {
                return self.fillState({})
            },
            set: function(obj) {
    ...
    cImport.prototype = {
        ...
        importS: function(sheet, extraRows, extraCols, keepCM, headerRowIndx) {
            ...
            for (i = 0, len = rows.length; i < len; i++) {
                ...
                row.htFix && (rd.pq_htfix = rd.pq_ht = row.htFix);
        ...
        importWb: function(obj) {
            var w = obj.workbook,
                ...
                sheet = obj.sheet,
                s, op = function(s) {
                    var tab = self.importS(s, obj.extraRows, obj.extraCols, obj.keepCM, obj.headerRowIndx);
                    tab.sheet = s;
                    tab.hidden = s.hidden;
                    tabs && tabs.push(tab);
                    return tab
                ...
                fn = function(options) {
                    main.options.dataModel.data = options.dataModel.data;
    ...
    cExport.prototype = $.extend({
            ...
            if (obj.format == "xlsx") {
                if (tabs) {
                    tabs.forEach(function(tab, id) {
                        if (instance = tab.instance || !(sheet = tab.sheet) && (instance = that.iTab.create(id))) {
        ...
        getXlsBody: function(CM, CMLen, data, dataLen, remotePage, offset, iMerge, render, iGV, shiftR, rowInit) {
            ...
            for (i = 0; i < dataLen; i++) {
                ...
                rowData.pq_htfix && (row.htFix = rowData.pq_htfix);
        ...
        getBody: function(rows, columns) {
            ...
            for (i = 0; i < rowsLen; i++) {
                ...
                for (j = 0; j < cellsLen; j++) {
                    ...
                    if (value == null) {
                        v = "<v></v>"
                    } else if (value == value * 1 && (value + "").match(/^[^0][0-9,e,\-,\.]*$/gi)) {
                        v = "<v>" + value + "</v>"
...
(function($) {
    ...
    $(document).on("pqGrid:bootup", function(evt, ui) {
        ...
        if (FM.on) {
            f = grid.iFormulas = new cFormulas(grid)
        }
    ...
    _pq.cEditor.prototype = {
        fixWidth: function(ui) {
            ...
            var $editor = ui.$editor,
                ...
                EM = $.extend({}, o.editModel, ui.column.editModel),
                $cont = that.$cont,
                posG = $cont.offset(),
                leftG = posG.left,
                topG = posG.top,
                ...
                $td.on("dblclick", function() {
                    ...
                    stop: function(evt) {
                        ...
                        var dy = evt.pageY - startPos,
                            arr = iR.rowhtArr;
                        self.$top.remove();
                        rd.pq_ht = rd.pq_htfix = Math.max(arr[rip] + dy, 10);
                        that.refresh()
                    }
                })
            }
